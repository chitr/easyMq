...
int  main(int argc,char **argv) {
	...
	start_writer_threads(&mq_info,no_of_threads,msg_count,msg_size);
	int rc=0;
	
	for( i=0;i<no_of_threads;i++) {
		PRINT(LOG_CRITICAL,"waiting to joint the thread %d",i);
		if(rc = pthread_join((mq_info[i]).mq_thread, &status[i]))
			PRINT(LOG_CRITICAL,"\n %d pthread_join()\n", i);
